...
# --- py3.10 pyd 本地裁剪:py3.8 行 138-163(26 行)在 pyd 中不存在 ---------------------
# [E1 probe_fe_1] pyd dir() 逐条实测 46 名,无 mulaw_encode_quantize_tf;
# [pyd strtab] 无 tensorflow / tf.variable_scope / tf.cast / tf.minimum / tf.sign / tf.log1p;
# [pyd line-tab] stft 的 co_firstlineno=165 与 py3.8 源定义行完全一致 ⇒ 本块行数被原样占位,
#   即 py3.8 的 def mulaw_encode_quantize_tf + 其体内 tf 语句 + 尾部注释块被整体删除/注释;
#   保留 26 行注释以维持后续全部函数的行号锚点(probe_fe_1:30/30 co_firstlineno 命中)。
# 被删内容(py3.8 原文,仅对照):
...
#   其后 py3.8 行 145-163 本为已注释的 mulaw_encode_tf / mulaw_decode_quantize_tf /
#   mulaw_decode_tf 片段(其中 153-157、160-163 在 py3.8 语法上仍属上一函数体)。
#
#
#
#
...
# ---------------------------------------------------------------------------
# R056 重建证据(注释追加在文件末尾,不影响任何函数 co_firstlineno)
#
# 源件(只读 oracle):modules/wenet/tools/_extract_feats.cp310-win_amd64.pyd
#   sha256 8e0e5f60064f3bdf84880ad013f09ca6fb081332940d239e9ae1647cd5261384
# 谱系/上游锚定(只读):
#   * /Volumes/A/数字人/szr2026/modules/heygem/wenet/tools/_extract_feats.py
#     (py3.8 明文,453 行)—— 本候选正文与该文件逐行一致,唯一差异是上面的 26 行 tf 裁剪块。
#   * 逐函数比对证据(evidence/modules/wenet__tools___extract_feats/runtime/probe_fe_1_out.json):
#     30/30 函数 co_firstlineno 与 py3.8 源 def 行号完全相同;
#     30/30 函数 co_varnames 与按 AST 从 py3.8 源收集的"形参+局部(源码序)"完全一致
#     (power_spec2mel 的 _mel_basis 是 global 声明,不进 co_varnames);
#     签名/默认值逐条一致(names.<fn>.signature / __defaults__)。
#   * 依赖实测(librosa 0.11.0 / torchaudio 2.7.1+cu118 / numpy 1.26.4 / scipy 1.15.3):
#     torchaudio 已无 load_wav(hasattr=False)⇒ _extract_feature(_norm) 在本环境走
#     AttributeError 路径;librosa.core.stft/istft、effects.split、filters.mel、
#     feature.delta、amplitude_to_db 均可用(probe_fe_1_out.json deps)。
# 行为以 E1 实测为准:30 个函数的动态观测由 VM 内 oracle 侧真实捕获,
#   见 golden/windows11/wenet__tools___extract_feats/ 与同批 results/。
