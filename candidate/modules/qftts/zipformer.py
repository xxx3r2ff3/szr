...
# -*- coding: utf-8 -*-
"""zipformer —— R054(T3:GPT-SoVITS/qftts 谱系)。

源件:modules/qftts/zipformer.cp310-win_amd64.pyd
      sha256 eed2c796058ad9d55cc3d7b26e6350c5c988312b586e4780fb28a82dfae3638c
对账材料:
  * 上游 icefall/zipvoice `models/modules/zipformer.py`
    (szr2026_out/bindiff/zipvoice_upstream/);
  * E1 探针 probe_surface.py / probe_behave.py。

本地改动核对结论(E1 实测)
--------------------------------
[E1-surface] 模块公开名集合与上游绑定名集合**完全一致**(38 名,无多无缺)。
[E1-surface] 唯一源码级差异是兄弟模块导入形态:
    `<-- 上游 --> from zipvoice.models.modules.scaling import (…)`
    `<-- 本地 --> from scaling import (…)`
    证据:oracle 侧 `ScaledLinear.__module__ == "scaling"`、
    `softmax.__module__ == "scaling"`、`Whiten/BiasNorm/ScheduledFloat.__module__
    == "scaling"`,且 zipformer 自身 dir() 中**没有** `zipvoice`/`scaling` 名
    (from-import 不绑定源模块名)。
[E1-surface] luxtts 内嵌明文版的 `DEVICE_TYPE` 在 oracle dir() 中不存在
    → 本地保留上游 `torch.amp.autocast("cuda", …)` 形态。
[E1-behave] `timestep_embedding`(dim 偶/奇、1-D/2-D timesteps、max_period、
    float64 入参)、`_whitening_schedule`(默认与 ratio=3)逐位一致。
[E1-behave] `TTSZipformer(8, 8)` 默认构造在 oracle 抛 `AssertionError`(空消息):
    上游 `_assert_downsampling_factor` 要求 factors[0]==1,而签名默认
    downsampling_factor=(2, 4) → **上游默认值本身即不可用**,本地同源保留。
    `warmup_batches=0` 时 `ScheduledFloat((0.0, 0.5), (0.0, 0.035))` 亦触发
    PiecewiseLinear 的严格递增断言(实测消息 `(0, (0.0, 0.5), (0.0, 0.035))`)。
"""
import copy
import logging
import math
...
# [本地改动] 上游为 `from zipvoice.models.modules.scaling import (…);`
# E1 实测 oracle 侧 ScaledLinear/softmax/Whiten/BiasNorm/ScheduledFloat 的
# __module__ 均为 "scaling",且模块级无 zipvoice/scaling 名 → 裸名导入。
from scaling import (
