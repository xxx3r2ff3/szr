...
# -*- coding: utf-8 -*-
"""scaling —— R049(T3:GPT-SoVITS/qftts 谱系)。

源件:modules/qftts/scaling.cp310-win_amd64.pyd
      sha256 707e2520c04a75823e3c89e3275c480187015aa7f290426a3b4de97b7c3628c3
对账材料(上游对齐 + 本地改动核对):
  * 上游 icefall/zipvoice `models/modules/scaling.py`
    (szr2026_out/bindiff/zipvoice_upstream/,即 R051 solver 同一 git 源的副本);
  * E1 探针:probe_surface.py(名字集/签名/类面)、probe_behave.py(逐点行为)。

本地改动核对结论(E1 实测)
--------------------------------
[E1-surface] 模块公开名集合 = 上游绑定名集合 **减去** `custom_amp_decorator`
    与 `deprecated`(oracle dir() 无此二名);其余 57 名逐一对应,无多无缺。
[E1-surface] `custom_fwd`/`custom_bwd` 在 oracle 中是 cyfunction `<lambda>`
    (`__module__ == "scaling"`),不是上游 `custom_amp_decorator` 返回的
    `<locals>.decorator`。
[E1-behave] 二者都是**单参数、无关键字参数**的 lambda:错用实测
    `TypeError: lambda1() takes no keyword arguments` /
    `lambda1() takes exactly one argument (0 given)` / `(2 given)`。
[E1-behave] probe_amp.py 逐点对拍:oracle `custom_fwd(f)` 与
    `torch.cuda.amp.custom_fwd(f)` **同一对象语义**(同 FutureWarning 文案、
    `__wrapped__` 同一、类型同为 function);`custom_bwd(f)` 与
    `torch.cuda.amp.custom_bwd(f)` 同理。torch 2.7.1 的 cuda.amp 侧签名是
    `custom_fwd(fwd=None, *, cast_inputs=None)`(**不收** device_type),
    由 deprecated 包装转发到 `torch.amp.custom_fwd(..., device_type="cuda")`
    ——这也解释了导入期的 FutureWarning。
    `custom_bwd(f)` 返回的 wrapper 在 forward 未走 autocast 时调用报
    `AttributeError: 'Tensor' object has no attribute '_fwd_used_autocast'`。
    → 本地等价实现(语义同上游,去掉了 helper 与 deprecated 变量):
        custom_bwd = lambda func: torch.cuda.amp.custom_bwd(func)
        custom_fwd = lambda func: torch.cuda.amp.custom_fwd(func)
[E1-behave] 其余 24 个 callable 的签名与行为逐点对齐上游(见 -impl.md §E1 表):
    logaddexp/logaddexp_onnx/softmax/_diag/_no_op/_whitening_metric/with_loss/
    limit_param_value/penalize_abs_values_gt/SwooshLForward/SwooshRForward 及
    8 个 `_test_*` 自检(固定 seed 后全部通过)。
[E1-behave] luxtts 内嵌明文版的 `DEVICE_TYPE`/`get_memory_allocated` 在 oracle
    dir() 中**不存在**,故本地未采用该分支,保留上游 `torch.amp.autocast("cuda", …)`。
[版本分歧] 未发现(名字集与行为均对齐;唯一系统性差异即上面的 lambda 化)。

行内标注约定:`[E1-surface]`/`[E1-behave]` 指向上面两类探针输出;
`[本地改动]` 标注与上游 bindiff 源的差异点。
"""
import logging
import math
import random
...
# [本地改动] 上游此处为 `custom_amp_decorator` + `deprecated` 分支:
#   custom_fwd = custom_amp_decorator(custom_fwd, deprecated)
#   custom_bwd = custom_amp_decorator(custom_bwd, deprecated)
# E1 实测 oracle 的 dir() 中**没有** custom_amp_decorator / deprecated,
# 且 custom_fwd/custom_bwd 是 cyfunction `<lambda>`(单参数、无关键字参数,
# 见 "lambda1() takes no keyword arguments")。probe_amp.py 对拍确认二者分别
# 等价于 torch.cuda.amp.custom_fwd(func) / custom_bwd(func)(同警告、同 __wrapped__)。
custom_bwd = lambda func: torch.cuda.amp.custom_bwd(func)
custom_fwd = lambda func: torch.cuda.amp.custom_fwd(func)
