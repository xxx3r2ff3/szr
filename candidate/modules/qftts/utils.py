# -*- coding: utf-8 -*-
"""utils —— qftts 训练/推理公共工具(R053 重建候选)。

出处与证据
----------
* [上游] 本模块是 k2-fsa/ZipVoice(git ``2f7326fbfe999a3ad179e3f1af82a424d4a62819``,
  Apache-2.0)``zipvoice/utils/common.py`` 的**逐行还原**;同族 R049/R051/R054 已用
  同一快照对账。上游源件:``szr2026_out/bindiff/zipvoice_upstream/zipvoice/utils/common.py``
  (sha256 ``9725bb980cfff0e5eae4abf491030b50fab3170cfb80eb18ed0719267639f599``,670 行)。
* [E1] 二进制 ``modules/qftts/utils.cp310-win_amd64.pyd`` 由该文件经 Cython 编译而成
  (安装名 ``utils``,故 ``__module__ == "utils"``,且无 ``utils.common`` 子模块)。
  oracle 侧实测见 ``evidence/modules/qftts__utils/runtime/probes/probe_utils_behave.py``
  及其输出 ``probe_utils_behave_out.json``(141 个探针点),本文件按实测逐条核对:

  - ``dir()`` 公开名 49 个,与上游 import 区**逐个对齐**(无 ``Optional``/``TextIO``/
    ``__all__``);``Any/Dict/List/Tuple/Union`` 与 ``typing`` 同名对象 ``is`` 同一;
  - ``GradScaler.__module__ == "torch.amp.grad_scaler"`` → 走上游 ``hasattr(torch.amp,
    "GradScaler")`` 分支;
  - ``Pathlike == typing.Union[str, pathlib.Path]``;
  - ``torch_autocast`` 是 ``contextlib.contextmanager`` 包装体(``hasattr(__wrapped__)``);
  - ``AttributeDict(a=1).__str__(indent=4)`` 生效、缺键 ``AttributeError: No such
    attribute '<k>'``、``copy()`` 回退为普通 dict;
  - ``MetricsTracker`` 无 kwargs 构造、``reduce`` 无 process group 时 ``ValueError``、
    ``__str__`` 对未知 ``utt_*`` 键 ``ValueError``;
  - ``make_pad_mask`` 2-D 入参 ``AssertionError``、空张量 ``RuntimeError``(torch 的
    ``max()`` 语义);``pad_labels([])`` ``ValueError``;``get_tokens_index`` 的
    ``durations`` 补尾与越界切片语义逐值一致;
  - ``condition_time_mask``/``_suffix`` 在 ``manual_seed(0)`` 下逐位一致;
  - ``setup_dist`` 的 env 前置写与三种失败文案(RuntimeError/ValueError)一致;
  - ``cleanup_dist`` 无 process group 时抛 ``AssertionError``;
  - ``get_env_info`` 十个键逐个一致(``zipvoice-path`` = ``Path(__file__).resolve()
    .parent.parent``)、``get_git_*`` 因 oracle 环境无 ``git`` 可执行文件恒为 ``None``。

兄弟模块接口(硬约束)
--------------------
R044 ``checkpoint`` 导入期 ``from utils import AttributeDict``、R047 ``model`` 需要
``condition_time_mask``/``get_tokens_index``/``make_pad_mask``/``pad_labels``/
``prepare_avg_tokens_durations``/``GradScaler`` —— 本文件全部导出,且 ``__module__``
不再是 oracle pyd。
"""
import argparse
import collections
...
# [E1] oracle 的 GradScaler.__module__ == "torch.amp.grad_scaler" → 上游这一分支成立。
