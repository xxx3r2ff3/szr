# -*- coding: utf-8 -*-
# base_function —— S004 由 py3.8 字节码逐条重解码恢复
# (来源:landmark2face_wy/models/base_function.pyc;反汇编
#  evidence/modules/_heygem_orphans/disasm/landmark2face_wy_models_base_function.pyc.dis,
#  20 个 code object;另用 xdis 对原 pyc 做了带行号的精确反汇编交叉核对)。
#
# 模块本身没有 docstring(co_consts[0] == 0),故出处说明写在此注释块里。
# 定义清单与源码行号(与 pyc 中 co_firstlineno 一一对应):
#   LayerNorm2d(11) / ADAINHourglass(31) / ADAINEncoder(43) / ADAINDecoder(64) /
#   ADAINEncoderBlock(92) / ADAINDecoderBlock(111) / spectral_norm(151) / ADAIN(159) /
#   FineEncoder(193) / FineDecoder(215) / FirstBlock2d(247) / DownBlock2d(266) /
#   UpBlock2d(284) / FineADAINResBlocks(298) / Jump(312) / FineADAINResBlock2d(327) /
#   FinalBlock2d(350) / __main__ 段(370)。
# 置信度:全部函数体均按 [Disassembly] 逐指令还原(每个 LOAD_CONST/STORE_ATTR/
# CALL_FUNCTION_KW 都对应到源码形态);docstring 逐字取自各 code object 的 co_consts[0]。
# S003 版的臆造骨架(与字节码签名/层结构不符)已按字节码整体替换。
import sys
...
class ADAINDecoder(nn.Module):
    ...
    def __init__(self, pose_nc, ngf, img_f, encoder_layers, decoder_layers, skip_connect=True,
        ...
        for i in range(encoder_layers - decoder_layers, encoder_layers)[::-1]:
            ...
            model = ADAINDecoderBlock(in_channels, out_channels, out_channels, pose_nc, use_transpose,
...
class ADAINDecoderBlock(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, hidden_nc, feature_nc, use_transpose=True,
                 nonlinearity=nn.LeakyReLU(), use_spect=False):
...
class ADAIN(nn.Module):
    ...
    def forward(self, x, feature):
        ...
        # apply the partially-instantiated normalization layer on the input
        normalized = self.param_free_norm(x)

        # Here we use the input as the style code, this is the only difference
...
if __name__ == '__main__':
    a = FineADAINResBlocks(3, 64, 64)
    b = torch.ones(1, 64, 128, 128)
    c = torch.ones(1, 73, 27)
    output = a(b, c)
    print(output.shape)
