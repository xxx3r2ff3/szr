# -*- coding: utf-8 -*-
# 出处:landmark2face_wy/options/base_options.pyc(py3.8,反汇编见
# evidence/modules/_heygem_orphans/disasm/landmark2face_wy_options_base_options.pyc.dis)。
# 该 .dis 的模块 [Constants] 第一项不是字符串 => 模块级没有 docstring,故出处写在注释里。
# 恢复说明:
#   * initialize() 的 63 个 parser.add_argument 调用(参数名/type/default/help)逐字取自
#     [Disassembly] 的 LOAD_CONST 与 CALL_FUNCTION_KW 关键字元组;
#   * 其余方法体按 [Disassembly] 逐指令等价重建(if/for 分支、STORE_ATTR 顺序与字节码一致);
#   * 未还原细节:无(所有语句均可由字节码直接确定)。
import argparse
import os
from landmark2face_wy.util import util
import torch
...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--audio_feature', help='audio feature extraction mode: mfcc, deepSpeech, wenet, 3dmm', default='3dmm', type=str)
        ...
        parser.add_argument('--perceptual_layers', type=list, default=['relu_1_1', 'relu_2_1', 'relu_3_1', 'relu_4_1', 'relu_5_1'], help='perceptual loss layers')
    ...
    def gather_options(self):
        ...
        """
        # check if there are new options
