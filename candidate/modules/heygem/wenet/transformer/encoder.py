# -*- coding: utf-8 -*-
# 出处:S003 恢复件(candidate/modules/heygem/wenet/transformer/encoder.pyc,py3.8)。
# 反汇编:evidence/modules/_heygem_orphans/disasm/wenet_transformer_encoder.pyc.dis
# 字节码全局名对照:AssertionError <- check_argument_types 断言与
# forward_chunk/forward_chunk_by_chunk 中 assert 语句隐式加载(py3.8)。
"""Encoder definition."""
...
class BaseEncoder(torch.nn.Module):
    ...
    def forward_chunk(
        ...
        # Real mask for transformer/conformer encoders
        masks = torch.ones(1, xs.size(1), device=xs.device, dtype=torch.bool)
        masks = masks.unsqueeze(1)
        r_elayers_output_cache = []
        r_conformer_cnn_cache = []
        for i, layer in enumerate(self.encoders):
            attn_cache = None if elayers_output_cache is None else \
                elayers_output_cache[i]
            cnn_cache = None if conformer_cnn_cache is None else \
                conformer_cnn_cache[i]
