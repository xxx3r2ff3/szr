# -*- coding: utf-8 -*-
# 出处:S003 恢复件(candidate/modules/heygem/wenet/transformer/encoder_layer.pyc,py3.8)。
# 反汇编:evidence/modules/_heygem_orphans/disasm/wenet_transformer_encoder_layer.pyc.dis
# 字节码全局名对照:AssertionError <- 两个 forward 中 output_cache 相关 assert
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        if self.concat_after:
            x_concat = torch.cat((x, self.self_attn(x_q, x, x, mask)),
                                 dim=-1)
        ...
        fake_cnn_cache = torch.tensor([0], dtype=x.dtype, device=x.device)
...
class ConformerEncoderLayer(nn.Module):
    ...
    def forward(self,
        ...
        new_cnn_cache = torch.tensor([0], dtype=x.dtype, device=x.device)
        if self.conv_module is not None:
            residual = x
            if self.normalize_before:
                x = self.norm_conv(x)
