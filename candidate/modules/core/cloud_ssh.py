# -*- coding: utf-8 -*-
"""modules.core.cloud_ssh —— T4R 语义重建(R035)。

源 pyd:modules/core/cloud_ssh.cp310-win_amd64.pyd
        sha256 e255574c5a5a246d2e51d7c392c611b6ee0fe0812f6a7378ca354d14a2e29d4d
...
二进制出处(Ghidra 12.1.3 反编译,证据 runtime/ghidra/):
  ...
  * 模块 exec @0x1800076c0;InitCachedConstants @0x180006e20(0x180012120 =
    ...
    [COM-D002] 这两个**凭据/原厂基础设施字面量的值已删除**(本文件及
    ssh_proxy.py 的同形副本一并清除):__main__ 改为从凭据保险箱/配置读取,
    缺失即显式报错,绝不回落字面量。行为要点(run_proxy/parse_ssh 等)不变。

E1 探针(evidence/modules/core__cloud_ssh/runtime/):
  probeA_env_ssh.py / probeA2_ssh_edges.py / probeB_ssh_stub.py。
...
def run_proxy(ssh_cmd: str, ssh_password: str, port: int = 9887):
    ...
    # i006_ssh_tunnel.ssh_run.oracle.json 的 run_proxy_orchestration 记录桩):
