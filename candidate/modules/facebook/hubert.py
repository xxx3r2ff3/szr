...
import librosa
import transformers  # For suppressing warnings
...
wav2vec2_processor = Wav2Vec2FeatureExtractor.from_pretrained(
    os.path.join(now_dir, "hubert-large-ls960-ft"),
)
...
def get_hubert_from_16k_speech(speech, device="cuda:0"):
    ...
    # 将输入数据转换为半精度
    input_values_all = input_values_all.half().to(device)
...
def make_hubert(wav_name):
    ...
    hu_file = wav_name.replace(".wav", "_hu.npy")
    np.save(hu_file, hubert_hidden.float().detach().numpy())
